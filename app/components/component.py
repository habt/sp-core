...
class Component():
    ...
    def update_sigma_delay(self):
        self.sigma_level = DEFAULT_SIGMA_LEVEL if self.sigma_level is None else self.sigma_level
        self.sigma_delay = self.pred_delay + self.stdv_delay * self.sigma_level
        self.delay_history.append(self.sigma_delay)

    def update_ewma_delay(self):
        if self.ewma_delay is not None:
            self.ewma_delay = (1 - self.ewma_alpha) * self.ewma_delay + self.ewma_alpha * self.sigma_delay
        else:
            self.ewma_delay = self.sigma_delay
